...
function getWeatherData($accessToken) {
	...
	if ( isset( $data->error ) ) {
		die( 'Netatmo API Error: ' . $data->error->message );
	}
 ...
 */
function print_temperatures( $stations ) {
...
function get_module_info( $module, $rain_module = false ) {
	...
	$recent_history_query = http_build_query(
		array(
			...
			'date_begin'   => strtotime( 'yesterday', time() ),
		)
	);
	...
	$time_24hrs_ago       = strtotime( 'today', time() );
		...
		if ( $rain_module ) {
			$rain_query = http_build_query(
				array(
					...
					'real_time'    => 'true',
					'type'         => 'sum_rain',
					'date_begin'   => strtotime( 'today', time() ),
					'limit'        => 100,
				)
			);
...
function get_yr_data() {
	...
	$yr_url = 'https://api.met.no/weatherapi/locationforecast/2.0/complete?altitude=' . $_ENV['ALTITUDE'] . '&lat=' . $_ENV['LATITUDE'] . '&lon=' . $_ENV['LONGITUDE'];
	...
	if ( isset( $_SESSION['forecast_expires'] ) && $_SESSION['forecast_expires'] < time() || empty( $_SESSION['forecast'] ) ) {
		$forecast_json                = file_get_contents( 'https://api.met.no/weatherapi/locationforecast/2.0/complete?altitude=60&lat=' . $_ENV['LATITUDE'] . '&lon=' . $_ENV['LONGITUDE'], false, $context );
